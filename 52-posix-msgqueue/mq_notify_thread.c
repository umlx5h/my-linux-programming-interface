...
notifySetup(mqd_t *mqdp)
{
    ...
    sev.sigev_notify = SIGEV_THREAD; // @3      /* Notify via thread */
    sev.sigev_notify_function = threadFunc;
    sev.sigev_notify_attributes = NULL;
            /* Could be pointer to pthread_attr_t structure */
    sev.sigev_value.sival_ptr = mqdp; // @4     /* Argument to threadFunc() */
...
main(int argc, char *argv[])
{
    ...
    mqd = mq_open(argv[1], O_RDONLY | O_NONBLOCK); // @5
    if (mqd == (mqd_t) -1)
        errExit("mq_open");

    notifySetup(&mqd);
    pause(); /* Wait for notifications via thread function */ // @6
}
