...
main(int argc, char *argv[])
{
    ...
    mqd = mq_open(argv[1], O_RDONLY | O_NONBLOCK); // @1
    if (mqd == (mqd_t) -1)
        errExit("mq_open");
    ...
    if (mq_getattr(mqd, &attr) == -1) // @2
        errExit("mq_getattr");

    buffer = malloc(attr.mq_msgsize); // @3
    if (buffer == NULL)
        errExit("malloc");

    /* Block the notification signal and establish a handler for it */

    sigemptyset(&blockMask); // @4
    ...
    sev.sigev_notify = SIGEV_SIGNAL; // @5
    ...
    for (;;) {
        sigsuspend(&emptyMask); /* Wait for notification signal */ // @6

        /* Reregister for message notification */

        if (mq_notify(mqd, &sev) == -1) // @7
            errExit("mq_notify");

        // キューを空にしないと通知を受け取れないのでノンブロッキングにして最後まで読み取る
        while ((numRead = mq_receive(mqd, buffer, attr.mq_msgsize, NULL)) >= 0) // @8
