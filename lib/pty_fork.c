        ...
        const struct termios *slaveTermios, const struct winsize *slaveWS)
{
    ...
    mfd = ptyMasterOpen(slname, MAX_SNAME); // @1
    if (mfd == -1)
        return -1;

    if (slaveName != NULL) { // @2      /* Return slave name to caller */
    ...
    if (setsid() == -1) // @5           /* Start a new session */
        err_exit("ptyFork:setsid");

    close(mfd); // @6                   /* Not needed in child */

    slaveFd = open(slname, O_RDWR); // @7 /* Becomes controlling tty */
    if (slaveFd == -1)
        err_exit("ptyFork:open-slave");

#ifdef TIOCSCTTY // @8                  /* Acquire controlling tty on BSD */
    if (ioctl(slaveFd, TIOCSCTTY, 0) == -1)
        err_exit("ptyFork:ioctl-TIOCSCTTY");
#endif

    if (slaveTermios != NULL) // @9     /* Set slave tty attributes */
        if (tcsetattr(slaveFd, TCSANOW, slaveTermios) == -1)
            err_exit("ptyFork:tcsetattr");

    if (slaveWS != NULL) // @10          /* Set slave tty window size */
        if (ioctl(slaveFd, TIOCSWINSZ, slaveWS) == -1)
            err_exit("ptyFork:ioctl-TIOCSWINSZ");

    /* Duplicate pty slave to be child's stdin, stdout, and stderr */

    if (dup2(slaveFd, STDIN_FILENO) != STDIN_FILENO) // @11
